import axios from 'axios';
import Actions from '../../redux/actionsTypes.js';
import { startLoading, stopLoading } from '../../components/Loading/actionCreators.js';
import { showAlert, hideAlert } from './../../components/AlertMessage/actionCreators.js';

const baseURL = 'http://192.168.1.13:8080/api/v1';

axios.defaults.headers.post['Content-Type'] = 'application/json';

export const sendVerificationCode = (email) => (dispatch) => {
    dispatch(startLoading());
    axios
        .post(`${baseURL}/auth/send_code_verification`, { email })
        .then((res) => {
            dispatch(stopLoading());
            return dispatch({ type: Actions.SEND_CODE_ACTIVATE_ACCOUNT_SUCCESS });
        })
        .catch((error) => {
            const { message } = error.response.data;
            dispatch(stopLoading());
            dispatch(showAlert(message[0]))
            return dispatch({
                type: Actions.SEND_CODE_ACTIVATE_ACCOUNT_ERROR,
            });
        });
};

export const verifyCode =
    (email = '', code = 0) =>
    (dispatch) => {
        dispatch(startLoading());
        axios
            .post(`${baseURL}/auth/codeVerification`, { email, code })
            .then((res) => {
                dispatch(stopLoading());
                return dispatch({ type: Actions.ACTIVATE_ACCOUNT_SUCCESS });
            })
            .catch((error) => {
                const { message } = error.response.data;
                dispatch(stopLoading());
                dispatch(showAlert(message[0]))
                return dispatch({
                    type: Actions.ACTIVATE_ACCOUNT_ERROR,
                });
            });
    };

export const clearActivateAccount = () => (dispatch) =>
    dispatch({ type: Actions.CLEAR_ACTIVATE_ACCOUNT });

export const showModal = () => (dispatch) => dispatch({ type: Actions.SHOW_MODAL });
export const hideModal = () => (dispatch) => dispatch({ type: Actions.HIDE_MODAL });
