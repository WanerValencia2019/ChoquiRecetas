...
export default function EmailVerification({ navigate, message, hide, show , reducer, email, press}) {
    ...
    return (
        <View style={styles.container}>
            ...
            {alertMessage.showAlert || state.error ? <AlertMessage message={alertMessage.message || state.message} /> : null}
            <View style={{ width: '100%', display: 'flex', alignItems: 'center' }}>
                <View style={{ display: 'flex', flexDirection: 'row', marginBottom: 10 }}>
                    <TextBasic text="code expires in " />
                    <TextBasic text={`0${minutes}:${seconds}`} type="Secondary" />
                </View>
                <Button text="Verify" height={50} containerStyle={{ width: '80%' }} press={()=>dispatch(press(email, value))} />
                <Button
                    text="Send again"
                    type="Light"
                    height={50}
            ...
            <Overlay  isVisible={state.showModal} overlayStyle={{margin: 15}} onRequestClose={()=>{
                dispatch(hide())
                navigate('login')
            }}>
